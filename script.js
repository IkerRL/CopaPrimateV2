...
document.addEventListener('click', (e) => {
    if (!syncToggleBtn.contains(e.target) && !syncPanel.contains(e.target)) {
...
if (initialRoom) {
    syncRoomInput.value = initialRoom;
    setTimeout(() => connectMQTT(initialRoom), 600);
} else {
...
function aplicarEstado(state) {
    ...
    if      (currentPhase === 'inicial')  renderInicial();
...
function renderSorteoState() {
    ...
    if (idx_sorteo >= total_sorteo && total_sorteo > 0) {
        ...
        if (infoEl) infoEl.textContent = '¡SORTEO COMPLETADO! 32 PARTIDOS';
